 ...
 * Marks a unit as completed (and viewed) for the authenticated user (in Postgres).
 */
export async function POST(request: NextRequest) {
  ...
  try {
    const { markCompleted } = await import("@/lib/db/queries");
    await markCompleted(payload.email, courseId, unitId);
  } catch {
